...
export class ArduinoUno extends Part {
  ...
  /**
   * Advance the MCU by one window (50 ms) in 16.667 ms slices, feeding inputs as
   * we go. `runForAsync` yields mid-slice so a slow slice can't freeze the tab
   * (avr8js runs well below real time). Tests set `syncMcu` to stay synchronous.
   */
  async afterSimulate() {
    ...
    for (let t = 0; t < simDuration; t += 16.667) {
      ...
      if (this.circuit.syncMcu) this.simulator.runFor(16.667)
      else await this.simulator.runForAsync(16.667)
    }
  }
